...
# export ARCH=rv_32imc
# export TEST=riscv_arithmetic_basic_test
# export SEED=0
...
# python3 ../dv_tests/scripts/spike_log_to_trace_csv.py --log ../dv_tests/dv_test_${ARCH}/${TEST}/seed_${SEED}/seed_${SEED}/out_*/spike_sim/* --csv iss.csv
...
export HOST=""    # set "_linux" if using linux system, and leave blank if using wsl
export ARCH=rv_32i
export TEST=Arithematic_basic_test
export SEED=0     #

#cleaning previous dump
rm mem.txt core.csv regr.log;
#running test
cp ../dv_tests/dv_test_${ARCH}/${TEST}/seed_${SEED}/test.hex mem.txt;

#run this one for atomic case only 
#cp ../../../test_a/mem.text mem.txt;             
#cp ../../../test_a/output.log trace_spike.log;
make questa$HOST 

cp      ../dv_tests/dv_test_${ARCH}/${TEST}/seed_${SEED}/seed_${SEED}/iss.csv .
python3 ../dv_tests/scripts/core_log_to_trace_csv.py --log trace_core* --csv core.csv 
#python3 ../dv_tests/scripts/spike_log_to_trace_csv.py --log trace_spike.log --csv iss.csv -v
#cp      ../../../test_a/iss.csv .
python3 ../dv_tests/scripts/instr_trace_compare.py --csv_file_1 iss.csv --csv_file_2 core.csv --csv_name_1 "iss" --csv_name_2 "core" --log regr.log 
mkdir -p output/$ARCH/${TEST}/${SEED}
[ ! -f regr.log ] && echo "regr.log file not create" && exit
...
if [ "${PASSED}" -eq 0 ]; then
 ...
 mv core.csv output/$ARCH/${TEST}/${SEED}
 mv regr.log output/$ARCH/${TEST}/${SEED}
# mv iss.csv css_files
# mv core.csv css_files 
# mv regr.log css_files