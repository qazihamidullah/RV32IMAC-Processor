...
FRD_RE = re.compile(r"(core\s+\d+:\s+)?(?P<pri>\d) 0x(?P<addr>[a-f0-9]+?) " \
                   "\((?P<bin>.*?)\) (.*\s.*) (?P<reg>[xf]\s*\d*?) 0x(?P<val>[a-f0-9]+)")
CORE_RE = re.compile(
...
def read_spike_trace(path, full_trace):
    ...
    with open(path, 'r') as handle:
        for line in handle:
            ...
            commit_match1 = FRD_RE.match(line)
            if commit_match or commit_match1:
                if commit_match:
                    ...
                    instr.gpr.append(gpr_to_abi(commit_match1.group('reg')
                                                .replace(' ', '')) +
                                    ':' + commit_match1.group('val'))
                    instr.mode = commit_match1.group('pri')
